...
export const ModalLowAccountsPayable = ({title, isOpen, setIsOpen, onClose, handleReturnModal, body}: TProp) => {
    ...
    const updateLow = async (currentBody: TAccountsPayable) => {
        try {
            const { status, data} = await api.put(`/accounts-payable/low`, currentBody, configApi());
