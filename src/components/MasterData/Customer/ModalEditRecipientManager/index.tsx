...
───────────────────────────────────────────── */
export const ModalEditRecipientManager = ({
    isOpen,
    recipientId,
    contractorType = "",
    onClose,
    ...
    const loadRecipient = async (contractorId: string) => {
        try { 
            const { data } = await api.get(`/customer-recipients/select?deleted=false&bond=Titular&contractorId=${contractorId}&orderBy=name&sort=asc&pageSize=200&pageNumber=1`, configApi()); 
    ...
    useEffect(() => {
        ...
        if(admin) {
            ...
            if(selectedContractorId) {
                loadRecipient(selectedContractorId);
            }
        } else {
            if(contractorId) {
                loadRecipient(contractorId);
            }
        }
    }, [watch("bond"), watch("contractorId")]);
                            ...
                            <div className="p-6 overflow-y-auto" style={{ maxHeight: "calc(100dvh - 22rem)" }}>
                                ...
                                {tab === "dados" && (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {
                                            admin && (
                                                <Field label="Contratante" span={2}>
                                                    <select className="select slim-select-primary flex-1" {...register("contractorId")}>
...

                                        {
                                            watch("bond") == "Dependente" && (
                                                <Field label="Titular" span={2}>
                                                    <select className="select slim-select-primary flex-1" {...register("bondId")}>
                                                        <option value="">Selecione</option>
