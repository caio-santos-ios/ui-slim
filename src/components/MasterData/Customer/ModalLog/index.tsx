...
export const ModalLog = ({parentId, onClose}: TProp) => {
    ...
    const getLogs = async () => {
        try {
            setLoading(true);
            const {data} = await api.get(`/logs?deleted=false&parentId=${parentId}&parent=customer&orderBy=createdAt&sort=desc&pageSize=100&pageNumber=1`, configApi());
    ...
    const normalizeTab = (tab: string) => {
        switch(tab) {
            case "customer-recipient": return "Beneficiários";
            case "contact": return "Contatos";
            case "attachment": return "Anexos";
            case "customer-contract": return "Contratos";
