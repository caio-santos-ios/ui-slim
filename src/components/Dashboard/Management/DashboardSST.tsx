...
export default function DashboardSST() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      // Calcula nota média por pergunta (campo chekinISOPoint contém score total)
      // O score ISO acumula os pontos de todas as Pn. Para simular por item,
      // distribuímos o ponto total proporcionalmente ao peso. 
      // Em produção, o backend deve salvar a resposta por item individualmente.
      // Por ora usamos chekinISOPoint para P11 (peso 3 = 9pts máx) como proxy.

      const notasPorItem = PERGUNTAS.map(p => {
        // Nota média sintética baseada nos vitals disponíveis
        const notaMedia = vitals.reduce((sum, v) => {
          const pontoISO = v.chekinISOPoint ?? 0;
          // heurística: distribui proporcionalmente ao peso
