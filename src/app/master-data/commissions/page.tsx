...
export default function Commissions() {
  ...
  const openModal = (action: "create" | "edit" = "create", body?: TCommissions) => {
    if(body) {
      const newBody = {...body}
  ...
  const openModalDelete = (body: TCommissions) => {
    setCurrentBody(body);
    setModalDelete(true);
  };
  ...
  const resetModal = () => {
    setCurrentBody(ResetCommissions);
      ...
      <Autorization />
      {
        ...
        <>
          ...
          <main className="slim-bg-main">
            ...
            <div className="slim-container w-full">
              <SlimContainer breadcrump="Comissões" breadcrumpIcon="MdPercent"
                ...
                <ul className="grid gap-2 slim-list-card lg:hidden">
                  {
                    pagination.data.map((x: TCommissions, i: number) => {
                      return (
                        <Card key={i}
                          buttons={
                            <>
            ...
            <ModalCommissions
              title={typeModal == 'create' ? 'Inserir Comissão' : 'Editar Comissão'} 
